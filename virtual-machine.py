...
def INC(file_in_map, instruction_pointer, first_arg, second_arg):
    file_in_map[second_arg:second_arg + block_size] = struct.pack('>i', struct.unpack('>i', file_in_map[
                                                                                            second_arg:second_arg + block_size])[0] + 1)
    file_in_map[0:block_size] = struct.pack('>i', instruction_pointer + block_size)
    return True
...
def IFLESSEQ(file_in_map, instruction_pointer, first_arg, second_arg):
    if (struct.unpack('>i', file_in_map[first_arg:first_arg + block_size])[0] <= struct.unpack('>i', file_in_map[
                                                                                                  second_arg:second_arg + block_size])[0]):
...
def execute_code(file_in_map):
    ...
    while not_stopped:
        ...
        not_stopped = globals()[function](file_in_map, instruction_pointer, first_arg, second_arg)
